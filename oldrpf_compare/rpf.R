...
rpf<- function(Y, X, max_interaction=2, ntrees=50, splits=30, split_try=10, t_try=0.4, variables=NULL, min_leaf_size=1, alternative=F, loss="L2", epsilon=0.1, deterministic= FALSE, categorical_variables=NULL, delta=0, cores=1){
  ...
  tree_fam <- function(run){
    ...
    while(splits>0){
      ...
      } else       R=Calc_Optimal_split2(Y,W, as.matrix(X), split_try, variables, individuals, min_leaf_size, split_candidates, loss, p+1, 0,delta,deterministic)
        ...
        if (min(length(I_2), length(I_1))>0){
          ...
          if(Ikx_opt[3] %in% variables[[Ikx_opt[1]]]){  ### if split variable is already in tree to be split
            ...
            for(i in 1:length(variables)){
              ...
              if((Ikx_opt[1]!=1 && length(variables[[i]])==length(variables[[Ikx_opt[1]]])+1) || (Ikx_opt[1]==1 && length(variables[[i]])==1)){ ###if number of variables in tree i is same as num of var in new split tree
                if((Ikx_opt[1]!=1 && all(variables[[i]]==sort(c(variables[[Ikx_opt[1]]],Ikx_opt[3])))) || (Ikx_opt[1] == 1 && variables[[i]]==Ikx_opt[3])){ ### if variables in tree i are same as  var in new split tree
