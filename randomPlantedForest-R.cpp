...
std::vector<std::vector<double>> to_std_vec(Rcpp::NumericMatrix rv) {
    return std::vector<std::vector<double>>(rv.begin(), rv.end());
}
...
class RandomPlantedForest {
    ...
    public:
        ...
        Rcpp::NumericVector predict(const NumericMatrix &X);
        void purify();
        // todo: getter/setter
...
Rcpp::NumericVector RandomPlantedForest::predict(const NumericMatrix &X){
    std::vector<std::vector<double>> input = to_std_vec(X);
    Rcpp::NumericVector res = from_std_vec(std::vector<double>{});
    return res;
}
