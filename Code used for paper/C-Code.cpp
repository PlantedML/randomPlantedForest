  ...
  for(int i_1=0; i_1<variables.size(); ++i_1) {
    ...
    for(int i_3=0; i_3<split_candidates.size(); ++i_3){
      ...
      for(int i_2=0; i_2<indiv.size(); ++i_2){
        ...
        if(samplepoints_1.size()<2*leaf_size(k)) continue;
        
        NumericVector samplepoints=NumericVector(samplepoints_1.size()+1-2*leaf_size(k));
        
        for(int i_5=0; i_5<samplepoints_1.size()+1-2*leaf_size(k); ++i_5){
          
          samplepoints(i_5)=samplepoints_1(i_5+leaf_size(k));
        }
