...
simulate_function <- function(Monte_Carlo){
  ...
  library(Rcpp)
  sourceCpp("PH-Testprogramm/C-Code_Number_3.cpp")
  library(interpret)
  library(survival)
  ...
  Data<-generate_data(n=500,p=10,rho=0.3,sparcity=10,sigma=1, Model=1, covariates='normal')
  ...
  splits=(p/2)*c(10,15,20,25,30)
  m=c(2,5,10,20)
  Itersplit_try=c(0.1,0.25,0.5,0.75)
  ...
  return(list(MSE_xgboost_additive=MSE_xgboost_additive,
...
#Results <- simulate_function(1)
stopCluster(cl)

save(Results,file="Model1-K=10_nosparce_Results3.Rdata")