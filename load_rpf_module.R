...
start_time <- Sys.time()
rpf_R_class <- rpf(y_train_class, x_train_class, max_interaction=max_inter, t_try=t_try, ntrees=n_trees, splits=n_splits, split_try=split_try, deterministic=deterministic_forest)
end_time <- Sys.time()
end_time - start_time
...
MSE_cpp_class <- rpf_cpp_class$MSE(predictions_cpp_class, y_test_class_class) 
