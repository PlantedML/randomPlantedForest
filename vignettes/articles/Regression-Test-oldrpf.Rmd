...
regression_comparer <- function(y, x, ntrees, max_interaction, splits, split_try, t_try, loss, delta, epsilon, lp = lp) {
  ...
  data.table(
    method = "regr", loss = "L2",
    misclass_R = misclass_R_reg, misclass_cpp = misclass_C_reg,
    diff = diff_reg, preds = list(pred_regr)
  )
}
...
classif_comparer <- function(y, x, ntrees, max_interaction, splits, split_try, t_try, loss, delta, epsilon, lop = lop) {
  ...
  pred <- data.table(Task = "classif", R = pred_r, Cpp = pred_cpp$.pred_1)

  data.table(
    method = "classif", loss = loss,
    misclass_R = misclass_R, misclass_cpp = misclass_C,
    diff = diff, preds = list(pred)
  )
}
...
cmp_regr <- regression_comparer(
  y = y, x = x, ntrees = ntrees, max_interaction = max_interaction,
  splits = splits, split_try = split_try, t_try = t_try, loss = loss, delta = delta, epsilon = epsilon, lp = lp
)
```
...
cmp_classif_l1 <- classif_comparer(
  y = y, x = x, ntrees = ntrees, max_interaction = max_interaction,
  splits = splits, split_try = split_try, t_try = t_try, loss = loss, delta = delta, epsilon = epsilon, lop = lop
)

cmp_classif_l1[["preds"]][[1]] |>
  ggplot(aes(x = R, y = Cpp)) +
  geom_point(size = 2, alpha = 2/3) +
  geom_abline(color = "red")
```
...
```{r classif-plot, fig.dim=c(9, 7)}
cmp_nested <- rbindlist(list(cmp_regr, cmp_classif_l1, cmp_classif_l2, cmp_classif_logit, cmp_classif_exp))

cmp_nested[, c("method", "loss", "misclass_R", "misclass_cpp", "diff")] |>
...
ggplot(cmp_long, aes(x = R, y = Cpp)) +
  facet_wrap(~Task + loss, scales = "free") +
  geom_point(size = 2, alpha = 2/3) +
  geom_abline(color = "red")

# Plot distributions
melt(cmp_long, measure.vars = c("R", "Cpp"), value.name = "Pred", variable.name = "Version") |>
