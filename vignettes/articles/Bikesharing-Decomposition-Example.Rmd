...
rp <- rpf(
  ...
  data = bike,
  max_interaction = 3, ntrees = 50
)

purify(rp)
```

We select the predictors of interest and use `predict_components()` to retrieve all predictive components that include them,
...
```{r extract-components, cache=TRUE, dependson="model-fit"}
vars <- c("hr", "temp", "workingday", "hum", "weathersit", "season")

components <- predict_components(rp, bike, predictors = vars)
