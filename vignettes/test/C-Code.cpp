...
NumericVector Calc_Optimal_split2(NumericVector Y, NumericVector W, NumericMatrix X, int split_try, List variables, List individuals, List leaf_size, List split_candidates, StringVector loss, std::vector<int>  categorical_columns, int max_categorical, double delta, bool deterministic) {
    ...
    for(int i_1=0; i_1<variables.size(); ++i_1) {
      ...
      for(int i_2=0; i_2<indiv.size(); ++i_2){
        ...
        if(deterministic){
          ...
          split_try=std::min(samplepoints_1.size()-2*leaf_size2 + 1,9.0);
        }
        ...
        for(int i_4=0; i_4<split_try; ++i_4){
          ...
          if(is_categorical) {
            ...
            if(deterministic) splitpoint = samplepoints(i_4);
            
            // Rcout << "Split point " << splitpoint << "\n";
