  ...
  if (length(i) == 1) {
    if (i == 0) {
      for (i in 1:length(forest_res[[3, s]])) {
        ###  tree 3=variables  --> i= trees

        variables <- forest_res[, s][[3]][[i]]

        if (length(forest_res[, s][[3]][[i]]) == 1) {
          if (forest_res[, s][[3]][[i]] == 0) next
        }

        for (j in 1:length(forest_res[[1, s]][[i]])) {
          ## 1= intervals --> j is leaf of tree i
      ...
      # f<-mean(f[-1])
      if (is.null(forest_res[, s]$constant)) {
        return(f)
      }
  ...
  for (i_0 in 1:length(forest_res[[3, s]])) {
    if (length(forest_res[[3, s]][[i_0]]) == length(i)) {
      if (prod(forest_res[[3, s]][[i_0]] == sort(i))) {
        ## variables in tree i_0  match i

        for (j in 1:length(forest_res[[1, s]][[i_0]])) {
          ### j is leaf in tree i_0
...
F_average <- function(x, forest_res, i = 0) {
  ...
  for (s in 1:ntrees) {
    y <- y + F_single_fam(x, s, i, forest_res)
  }
...
predict_rpf <- function(X, forest_res, i = 0) {
  if (length(i) == 1) {
    if (i == 0) {
      ...
      if (length(X) == p) {
        return(F_average(X, forest_res))
      }

      if (!is.matrix(X)) {
        return("The input X has the wrong dimension in order to calculate f(x)")
      }
  ...
  if (is.null(forest_res[, 1]$constant)) {
    print("Note: The estimator has not been purified.")
  }

  if (length(X) == length(i)) {
    return(F_average(X, forest_res, i = i))
  }
  ...
  if (dim(X)[2] != length(i)) {
    return("The input X has the wrong dimension in order to calculate f_i(x)")
  }
